...
import { MoreVertical, Check, X, MapPin, Loader2, Trash2, ExternalLink, Pencil, Camera } from "lucide-react";
...
export function AdminPropertyTable({
    ...
    const handleAction = (id: string, action: string, property?: AdminProperty) => {
        ...
        if (action === "approve") {
            ...
            onDelete(id);
        } else if (action === "edit" && onEdit && property) {
            onEdit(property);
        }
    };
                    ...
                    properties.map((property) => (
                        <div key={property.id} className="p-4 flex flex-col gap-4 hover:bg-slate-50/50 transition-colors">
                            <div className="flex gap-4">
                                <div className="w-20 h-20 rounded-xl bg-slate-100 flex items-center justify-center shrink-0 border border-border overflow-hidden">
                                    {property.media?.[0] ? (
                                        <Image src={property.media[0].url} alt={property.title} width={80} height={80} className="w-full h-full object-cover" unoptimized />
                                    ) : (
                                ...
                                <div className="flex flex-col gap-1 min-w-0">
                                    ...
                                    <span className="text-primary font-semibold text-xs mt-1">
                                        {formatPrice(property.price)} · {property.property_type.toUpperCase()}
                                    </span>
                                </div>
                            </div>
                                ...
                                <div className="flex items-center gap-1.5 ml-auto">
                                    {isLoading(property.id) ? (
                                        ...
                                        <>
                                            ...
                                            {onEdit && (
                                                <button
                                                    onClick={() => handleAction(property.id, "edit", property)}
                            ...
                            properties.map((property) => (
                                <tr key={property.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-4 min-w-0 py-0.5">
                                            <div className="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center shrink-0 border border-border overflow-hidden">
                                                {property.media?.[0] ? (
                                                    <Image src={property.media[0].url} alt={property.title} width={48} height={48} className="w-full h-full object-cover" unoptimized />
                                                ) : (
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                ...
                                                <div className="flex items-center text-text-muted text-[11px] mt-0.5">
                                                    <MapPin className="w-3 h-3 mr-1 shrink-0" />
                                                    <span className="truncate">{property.locality ? `${property.locality}, ` : ""}{property.city}</span>
                                                </div>
                                                <div className="text-primary font-bold text-[11px] mt-1 flex items-center gap-1.5">
                                                    {formatPrice(property.price)}
                                                    <span className="w-1 h-1 rounded-full bg-border" />
                                                    <span className="text-text-secondary font-medium uppercase tracking-tighter">{property.property_type}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
